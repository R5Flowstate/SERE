...
// Oodle lives in the game's own oo2core DLL. Bound lazily so a missing DLL
// degrades to "cannot decompress" rather than blocking launch.
namespace {
	typedef intptr_t(__stdcall* OodleLZ_Decompress_t)(
	...
	OodleLZ_Decompress_t ResolveOodle()
	{
		static OodleLZ_Decompress_t fn = nullptr;
		...
		for (const char* name : candidates) {
			...
			if (!mod) continue;
			fn = (OodleLZ_Decompress_t)GetProcAddress(mod, "OodleLZ_Decompress");
			if (fn) {
				printf("[SERE] Oodle bound: %s\n", name);
				fflush(stdout);
				return fn;
			}
		}
		printf("[SERE] Oodle unavailable: place oo2core_8_win64.dll next to SERE.exe\n");
		fflush(stdout);
		return nullptr;
	}
}

size_t RTech::OodleDecompress(const void* src, size_t srcLen, void* dst, size_t dstLen)
{
	OodleLZ_Decompress_t oodle = ResolveOodle();
	if (!oodle) return 0;

	intptr_t written = oodle(src, (intptr_t)srcLen, dst, (intptr_t)dstLen,
		1, 0, 0, nullptr, 0, nullptr, nullptr, nullptr, 0, 3);
	if (written <= 0) {
		printf("[SERE] Oodle decompress failed (in=%zu expected=%zu)\n", srcLen, dstLen);
		fflush(stdout);
		return 0;
	}
